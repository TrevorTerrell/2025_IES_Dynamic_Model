package MCFR_HyS_BOP
  ...
  model Simbase
    ...
    MCFR_HyS_BOP.Nuclear.ReactivityFeedback ReactivityFeedback(a_F = -4.4376e-05, a_R = 0.1164e-05, step_mag = -185*1e-5, omega = 0, sin_mag = 1*1e-5, ramp_rate = -1e-5, ramp_low = 0.4, stepInsertionTime = 500000000, sinInsertionTime = 88888888, rampInsertionTime = 500000000) annotation(
...
//n_population.n = 1;
    ...
    model Poisons
      ...
      parameter MCFR_HyS_BOP.Units.IsotopicFissYield gamma_I = 0.06390; // Fraction of fissions that directly produce this isotope
      ...
      parameter MCFR_HyS_BOP.Units.MicroAbsorptionCrossSection sig_Xe = 5.915e-24;//2.66449e-18;
      ...
      parameter MCFR_HyS_BOP.Units.OffgasRemovalRate lam_bubble;       // Removal rate of fission gases via bubble removal
    ...
    model ReactivityFeedback
      ...
      parameter MCFR_HyS_BOP.Units.TemperatureReactivityCoef a_F; // Fuel temp feedback coefficient
      parameter MCFR_HyS_BOP.Units.TemperatureReactivityCoef a_R; // Reflector temp feedback coefficient
    ...
    equation
      ...
      ReactExternalSin = delay(sin_mag*sin(omega*time), sinInsertionTime); //0 + (if time < sinInsertionTime then 0 else sin_mag*sin(omega*time)); //- (sinInsertionTime-1.5697)/omega)-sin_mag);
    //  ReactExternalRamp = 0 + (if time < rampInsertionTime then 0 else ramp_rate*time);
//  if time < rampInsertionTime then
//  ReactExternalRamp = 0;
//  elseif time < (rampInsertionTime + (dollar*ramp_low/ramp_rate)) then
//  ReactExternalRamp = ramp_rate * (time - rampInsertionTime);
//  else
//  ReactExternalRamp = -ramp_rate * (time - rampInsertionTime - (dollar*ramp_low/ramp_rate));
//  end if;
//ReactExternalRamp = 0 + (if time < rampInsertionTime then 0 else if time < (rampInsertionTime + 300) then (time - rampInsertionTime)*-0.00296/300 else height);
      ReactExternalRamp = 0 + (if (time < rampInsertionTime) then 0 else if (time < rampInsertionTime + 54.55) then (600*1e-5)*(time - rampInsertionTime)/54.55 else if (time < rampInsertionTime + 1050000) then (600*1e-5) else if (time < rampInsertionTime + 1100000) then (600*1e-5)*(rampInsertionTime + 1150000 - time)/54.55 else 0);
      
    //  ReactExternalRamp = 0 + (if (time < rampInsertionTime) then 0 else if (time < rampInsertionTime + 500) then -0.00444*(time - rampInsertionTime)/500 else if (time < rampInsertionTime + 1500) then -0.00444 else if (time < rampInsertionTime + 4000) then -0.00444*(rampInsertionTime + 4000 - time)/2500 else 0);
    //  ReactExternalRamp = offset + (if (time < startTime or nperiod == 0 or (nperiod > 0 and count >= nperiod)) then 0 else if (time < T_start + T_rising) then amplitude*(time - T_start)/rising else if (time < T_start + T_width) then amplitude else if (time < T_start + T_falling) then amplitude*(T_start + T_falling - time)/falling else 0);
  ...
  package HeatTransport
    model Core
      ...
      parameter MCFR_HyS_BOP.Units.ResidentTime tauCoreToPHX = 1;//0.6887;
...
// These next two are the simpler version for constant htc
    ...
    model OnceThroughSteamGenerator
      ...
      // Parameter declaration
      //parameter MCFR_HyS_BOP.Units.Mass M_sh_initial; // Starting total superheated steam mass
      ...
      parameter MCFR_HyS_BOP.Units.Density rho_sh = 45.05219; 
      ...
      parameter MCFR_HyS_BOP.Units.SpecificHeatCapacity cP_SH1 = 0.002573333673413; 
      parameter MCFR_HyS_BOP.Units.SpecificHeatCapacity cP_SH2 = 0.003687517297276; 
      parameter MCFR_HyS_BOP.Units.SpecificHeatCapacity cP_fdw = 0.004393935709417; // Specific heat of subcooled fluid at OTSG inlet
      ...
      //parameter MCFR_HyS_BOP.Units.MassFlowRate m_dot_BNom; // Saturated boiling domain nominal flow rate
      ...
      //  parameter MCFR_HyS_BOP.Units.Temperature T_fdw_initial = 180;
      //  parameter MCFR_HyS_BOP.Units.Temperature T_fdw = 180;
      parameter MCFR_HyS_BOP.Units.Temperature T_PN1_OTSG_initial = 605.5;
      parameter MCFR_HyS_BOP.Units.Temperature T_PN2_OTSG_initial = 579.6;
      parameter MCFR_HyS_BOP.Units.Temperature T_PN3_OTSG_initial = 557.5;
      parameter MCFR_HyS_BOP.Units.Temperature T_PN4_OTSG_initial = 537.3;
      parameter MCFR_HyS_BOP.Units.Temperature T_PN5_OTSG_initial = 523.9;
      parameter MCFR_HyS_BOP.Units.Temperature T_PN6_OTSG_initial = 507.3;
      parameter MCFR_HyS_BOP.Units.Temperature T_TN1_OTSG_initial = 583.4;
      parameter MCFR_HyS_BOP.Units.Temperature T_TN2_OTSG_initial = 542.1;
      parameter MCFR_HyS_BOP.Units.Temperature T_TN3_OTSG_initial = 408.7;
      parameter MCFR_HyS_BOP.Units.Temperature T_TN4_OTSG_initial = 401.0;
      parameter MCFR_HyS_BOP.Units.Temperature T_TN5_OTSG_initial = 416.0;
      parameter MCFR_HyS_BOP.Units.Temperature T_TN6_OTSG_initial = 368.8;
      parameter MCFR_HyS_BOP.Units.Temperature T_SH1_OTSG_initial = 594.7;
      parameter MCFR_HyS_BOP.Units.Temperature T_SH2_OTSG_initial = 537.6;
      parameter MCFR_HyS_BOP.Units.Temperature T_sat_OTSG_initial = 327.8;
      parameter MCFR_HyS_BOP.Units.Temperature T_SC2_OTSG_initial = 250.1;
      ...
      parameter MCFR_HyS_BOP.Units.Length L_sc_initial = 3;//1.4469*(23)^(1/3);
      parameter MCFR_HyS_BOP.Units.Length L_b_initial = 3;//2.3645*(23)^(1/3);
      parameter MCFR_HyS_BOP.Units.Length L_sh_initial = 4;//4.723*(23)^(1/3);
      parameter MCFR_HyS_BOP.Units.Length L_OTSG = 10;//8.5344*(23)^(1/3);
    ...
    //  MCFR_HyS_BOP.Units.MassFlowRate m_dot_fdw;
    ...
    //  MCFR_HyS_BOP.Units.MassFlowRate m_dot_SH;
    //  MCFR_HyS_BOP.Units.Temperature T_fdw;
    ...
    //  MCFR_HyS_BOP.Units.Temperature T_SH1;
    ...
    //  MCFR_HyS_BOP.Units.Pressure P_stm;
...
//  der(L_sh) = 0;
    ...
    //  m_dot_SH.mdot = 388.963;
//  m_dot_SC = m_dot_fdwNom;
//  m_dot_B = m_dot_fdwNom;
//  m_dot_SH = m_dot_fdwNom;
    //  m_dot_SH.mdot = 192.5;//415;
    //  m_dot_fdw.mdot = 192.5;
    //  m_dot_SC = 415;
    equation
...
//  if T_SH2 > 50 then
      ...
      der(T_SH2)*(M_sh*0.5*cP_SH2) = (h_wsh*(N_tubes*pi*D_outer*L_sh*0.5)*(T_TN2 - T_SH2) + (m_dot_B*cP_SH2*(T_sat - T_SH2)) + (A_flow*L_sh*0.5 - dHs_dPs)*der(P_stm.P)); // Superheated steam node #2 temperature eqn
    
    //  der(T_SH1.T)*(M_sh*0.5*cP_SH1) = (h_wsh*(N_tubes*pi*D_outer*L_sh*0.5)*(T_TN1 - T_SH1.T) + (m_dot_SH.mdot*cP_SH1*(T_SH2 - T_SH1.T)) + (cP_SH1*(T_SH2 - T_SH1.T)*der(M_sh)) + (A_flow*L_sh*0.5 - dHs_dPs)*der(P_stm.P)); // Superheated steam node #1 (outlet) temperature eqn
    //  der(T_SH2)*(M_sh*0.5*cP_SH2) = (h_wsh*(N_tubes*pi*D_outer*L_sh*0.5)*(T_TN2 - T_SH2) + (m_dot_B*cP_SH2*(T_sat - T_SH2)) + (A_flow*L_sh*0.5 - dHs_dPs)*der(P_stm.P)); // Superheated steam node #2 temperature eqn
      
    //  der(T_SH1.T)*(M_sh*0.5*cP_SH1) = (h_wsh*(N_tubes*pi*D_outer*L_sh*0.5)*(T_TN1 - T_SH1.T) + (m_dot_SH.mdot*cP_SH1*(T_SH2 - T_SH1.T)) + (cP_SH1*(T_SH2 - T_SH1.T)*der(M_sh)*0.5) + (A_flow*L_sh*0.5 * dHs_dPs)*der(P_stm.P)); // Superheated steam node #1 (outlet) temperature eqn
    //  der(T_SH2)*(M_sh*0.5*cP_SH2) = (h_wsh*(N_tubes*pi*D_outer*L_sh*0.5)*(T_TN2 - T_SH2) + (m_dot_B*cP_SH2*(T_sat - T_SH2)) + (A_flow*L_sh*0.5 - M_sh*0.5*dHs_dPs)*der(P_stm.P)); // Superheated steam node #2 temperature eqn
...
//  end if;
    //  der(T_SC2) = ((h_wsc*(N_tubes*pi*D_outer*L_sc*0.5)*(T_TN6-T_SC2) + (cP_fdw*m_dot_fdw.mdot*180) - cP_SC*((m_dot_fdw.mdot+m_dot_SC)*0.5*T_SC2) + A_flow*L_sc*0.5*der(P_stm.P)) * (2/(A_flow * rho_SC *cP_SC)) - T_SC2*der(L_sc))*(1/L_sc); // Subcooled fluid node temperature eqn
...
//  der(P_stm) = 0;
      ...
      //der(P_stm.P)*A_flow*L_sh = ((Z_ss*R_ugc)*(der(M_sh)*(T_SH1.T + T_SH2) + M_sh*(der(T_SH1.T) + der(T_SH2)))/(2*MM_stm)) - (P_stm.P*A_flow*der(L_sh)); // Secondary side pressure. T_sh1 is OTSG outlet. Assumption: P_subcooled = P_stmuratedboiling = P_superheated
      der(P_stm.P)*A_flow*L_sh = ((Z_ss*R_ugc)*(der(M_sh)*(T_SH2) + M_sh*(der(T_SH2)))/(MM_stm)) - (P_stm.P*A_flow*der(L_sh)); 
      
      rho_b = 25.884875705 + 12.835*P_stm.P; // Two-phase boiling density eqn
...
//  m_dot_B = 1.925004320027648e+02;
    ...
    //======================
      //PARAMETERS:
      //parameter MCFR_HyS_BOP.Units.Pressure P_HP = 12.5;//INPUT FROM OTSG
    ...
    //parameter MCFR_HyS_BOP.Units.MassFlowRate m_dot_OTSG = 192.5;//INPUT FROM OTSG
      //parameter MCFR_HyS_BOP.Units.Temperature T_OTSG_initial = 600;//INPUT FROM OTSG
    parameter MCFR_HyS_BOP.Units.Number K_RH = 0.2;//0.16;
    parameter MCFR_HyS_BOP.Units.Number K_HPB = 0.02;//0.1634;
    parameter MCFR_HyS_BOP.Units.Number K_LPB_initial = 0.12;    //0.2174;
      //parameter MCFR_HyS_BOP.Units.SpecificEnthalpy H_HPT_in = Water.specificEnthalpy_pT(P_HP*1e6, T_OTSG+273.15)/1e6;
    //parameter MCFR_HyS_BOP.Units.SpecificEnthalpy H_HPT_in_initial = Water.specificEnthalpy_pT(P_HP*1e6, T_OTSG_initial + 273.15)/1e6;
    parameter MCFR_HyS_BOP.Units.Temperature T_setpoint = 180;//212;
    parameter MCFR_HyS_BOP.Units.Temperature T_cool_in = 22;
    parameter MCFR_HyS_BOP.Units.MassFlowRate m_dot_cool;
    ...
    //  MCFR_HyS_BOP.Units.SpecificEnthalpy H_test;
      //===
    ...
    //===
      //MCFR_HyS_BOP.Units.Temperature T_OTSG;
...
//T_OTSG = T_OTSG_initial;
    ...
    //m_dot_fdw.mdot = 192.5;//415;
//  K_LPB = K_LPB_initial;
//  Q_RH = 50;
    //m_dot_fdw.mdot = 388.963;
    P_HP.P =12.5;
    equation
...
//VALVE AND NOZZLE CHEST EQNS:
       m_dot_v = m_dot_OTSG.mdot*K_RH;
//======================
//HIGH PRESSURE TURBINE EQNS:
       m_dot_HPT_in = m_dot_OTSG.mdot - m_dot_v;
...
//======================
    ...
    //  parameter MCFR_HyS_BOP.Units.MolFlowRate Mdot_SA_nom = 62309;
    //  parameter MCFR_HyS_BOP.Units.MolFlowRate Mdot_H2O_nom = 37691;
    ...
    //  parameter MCFR_HyS_BOP.Units.Mass m_vap = 100;
    ...
    //  parameter MCFR_HyS_BOP.Units.Area A1_inner = 5;
    ...
    //  parameter MCFR_HyS_BOP.Units.Temperature T_H_in = 800;//600;
      ...
      parameter MCFR_HyS_BOP.Units.pressure_kPa pressure;// = 100;// 1500// Secondary side pressure [kPa]
    ...
    //  parameter MCFR_HyS_BOP.Units.FlowFraction FF_HyS_Hot = 1;
    ...
    // Mass Balance
      ...
      Mdot_SA_in = Mdot_SA_nom + delay(sinmag*Mdot_SA_nom*sin(omega*time),sinstart) + (if (time < rampstart) then 0 else if (time < rampstart + 500) then -(Mdot_SA_nom*(1-ramp_min))*(time - rampstart)/500 else if (time < rampstart + 1500) then -(Mdot_SA_nom*(1-ramp_min)) else if (time < rampstart + 2000) then -(Mdot_SA_nom*(1-ramp_min))*(rampstart + 2000 - time)/500 else 0);
    // ~~~
